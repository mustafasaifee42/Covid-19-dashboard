...
const Sidebar: React.FunctionComponent<{
  ...
  let dataArr: any = Object.keys(props.data).map((key: string) => {
    return {
      countryName: key,
      confirmed:
        props.data[key]["confirmedData"][
          props.data[key]["confirmedData"].length - 1
        ]["value"],
      confirmedPer1000:
        props.data[key]["confirmedData"][
          props.data[key]["confirmedData"].length - 1
      ...
      death:
        props.data[key]["deathData"][props.data[key]["deathData"].length - 1][
          "value"
        ],
      recovery:
        props.data[key]["recoveryData"][
          props.data[key]["recoveryData"].length - 1
        ]["value"]
    };
  });
  dataArr.sort((x: any, y: any) =>
    d3.descending(x[props.sorted], y[props.sorted])
  );
  if (props.sorted !== "confirmed") {
    dataArr.sort((x: any, y: any) =>
      d3.descending(
        (x[props.sorted] * 100) / x.confirmed,
        (y[props.sorted] * 100) / y.confirmed
      )
    );
  }
  ...
  useEffect(() => {
    ...
    dataArr.sort((x: any, y: any) =>
      d3.descending(x["confirmed"], y["confirmed"])
    );
    ...
    let epidemicCurveData = countryList.map((d: string) =>
      props.data[d].confirmedData.filter((d: any) => d.value >= 100)
    );
    if (countryList.indexOf(props.country) >= 0) {
      ...
      for (
        let i = 10;
        i < props.data[countryList[0]].confirmedData.length + 1;
        i = i + 10
      ) {
        g.append("text")
          .attr("fill", "#999")
      ...
      g.selectAll(".doublingLines")
        ...
        .attr("x1", 0)
        .attr("x2", (d: number) =>
          x((d - 1) * doublingValue + doublingValue + 1)
        )
        .attr("y1", height)
      ...
      g.selectAll(".doublingLabel")
        ...
        .attr(
          "x",
          (d: number) => x((d - 1) * doublingValue + doublingValue + 1) - 10
        )
      ...
      let epidemicCurveDataFiltered = epidemicCurveData.filter(
        (d: any, i: number) => i < 12
      );
      let countryListFiltered = countryList.filter(
        (d: any, i: number) => i < 12
      );
      if (props.country !== "World") {
        epidemicCurveDataFiltered = [
          epidemicCurveData[countryList.indexOf(props.country)],
          epidemicCurveData[0]
        ];
      ...
      if (epidemicCurveDataFiltered[0].length > 1) {
        epidemicCurveDataFiltered.forEach((el: any, k: number) => {
          let countryG = g
            ...
            .on("mouseenter", () => {
              ...
              g.selectAll(".pandemicText")
                .attr("opacity", 0.1)
                .attr("fill", "#aaa");
              g.selectAll(`.connectorLine`).attr("opacity", 0.1);
              g.selectAll(`.curve${k}`)
                .attr("opacity", 1)
                .attr("stroke", "#e01a25");
          ...
          labelG
            ...
            .text(() => {
              let index = CountryNameData.findIndex(
                (obj: any) => obj.countryName === countryListFiltered[k]
              );
              return index < 0
                ? countryListFiltered[k]
                : CountryNameData[index].countryShortCode;
            });
        });
      } else {
        g.append("text")
  ...
  let style = props.bigScreen
    ? `calc(100vh - 280px - ${props.graphHeight}px)`
    : "auto";
  return (
    <div>
      <DataCards
